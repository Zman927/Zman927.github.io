...
- 웹 해킹 교육 플랫폼(캡스톤) / 쇼핑몰 프로젝트의 실제 GitHub 저장소 URL 미확인.
  이력서에는 저장소명이 없음 — 실제 링크가 있으면 알려주시고, 없으면 링크 없이
  텍스트 카드로만 소개한다.
- 익명 게시판 프로젝트는 GitHub 푸시 후 링크 연결 (4.1절)
